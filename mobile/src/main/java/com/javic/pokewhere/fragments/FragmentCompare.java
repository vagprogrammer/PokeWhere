...
public class FragmentCompare extends Fragment implements AdapterPokemonBank.ClickListener, DragSelectRecyclerViewAdapter.SelectionListener, OnViewItemClickListenner {
    ...
    public void onPrepareOptionsMenu(Menu menu) {
            if (mAdapterCompare.getSelectedCount() > 0) {
                ...
                this.menu.findItem(R.id.action_select_all).setVisible(true);
            } else {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.action_select_all:
                if (isAllSelected){
                    isAllSelected=false;
                    menu.findItem(R.id.action_select_all).setIcon(R.drawable.ic_check_box_white_24dp);
                    canFinish();
                }else{
                    isAllSelected=true;
                    menu.findItem(R.id.action_select_all).setIcon(R.drawable.ic_check_box_outline_blank_white_24dp);
                    mAdapterCompare.selectAll();
                }
        ...
        switch (task) {
            ...
            case Constants.ACTION_REFRESH_USER_DATA:
                ...
                if (mLocalUserPokemonList.size() > 0) {
                    ...
                    mToolbar.setTitle(String.valueOf(mLocalUserPokemonList.size()) + " " + localUserPokemon.getName());
                    if (((Integer)object==Constants.FRAGMENT_COMPARE)){
                        Toast.makeText(mContext, mContext.getString(R.string.text_no_pokemon), Toast.LENGTH_SHORT).show();
                        getActivity().onBackPressed();
                    }
                }
